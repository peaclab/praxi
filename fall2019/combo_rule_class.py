...
class RuleBasedTags:
    ...
    def fit(self,X,y, max_num_rules=5):
        ...
        for lab in unique_y:
            ...
            cur_other_lab.remove(lab) # other labels
            #print(indices)
            for i in range(1, length(cur_int)):
                if(rules_dic[lab] > max_num_rules):
                    break
                for combo in combinations(cur_int, i):
                    unique_rule = True
                    for y in cur_other_lab:
                        if all(x in intersections[y] for x in combo):
                            unique_rule = False
                            break
                    if unique_rule:
                        rules_dic[lab].append(combo)
        return rules_dic
    ...
    def predict(self, X, rel_rules):
        ...
        for changes in X:
            ...
            for key in rule_keys:
                if len(rel_rules[key]) != 0:
                    for rule in rel_rules[key]: # and set(rel_rules[key]) <= set(changes):
                        if set(rule) <= set(changes):
                        preds.append(key)
                        given_lab = True
                if given_lab = True:
                    break
