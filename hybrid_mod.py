...
class Hybrid(BaseEstimator):
    ...
    def __init__(self, freq_threshold=1, vw_binary='/home/ubuntu/bin/vw',
                 ...
                 vw_modelfile=None): # if this is set true it will delete everything after end of runtime?
    ...
    def fit(self, X, y, existing_model=None, idx_dic=None): # X = TAGsets, y = labels
        """ Trains classifier
        ...
        if not (self.iterative and self.trained):
            ...
            self.vw_oldmodel = existing_model
            print("Using old vw_modelfile: %s", self.vw_modelfile)
        else:
            print("Using old vw_modelfile: %s", self.vw_modelfile)
        ...
        if existing_model is not None:
            ...
            self.reverse_labels = idx_dic['reverse_labels']
        ...
        #save label info to a file
        if self.iterative:
            idx_dict = {'indexed_labels':self.indexed_labels, 'reverse_labels':self.reverse_labels,
                        'all_labels':list(self.all_labels), 'label_counter':self.label_counter}
            idx_file = '/home/ubuntu/praxi/idx_file.yaml'
            with open(idx_file, 'w') as outfile:
                yaml.dump(idx_dict, outfile, default_flow_style=False)
