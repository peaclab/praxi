#!/usr/bin/env python3
""" A cleaned up version of the research code used to generate the
    results in the Praxi paper. Note that the exact parameters for vw, etc.
    may be different from the ones used to generate paper results. This file
    just shows the cross validation framework that was used.
"""
...
def get_changeset(csid, iterative=False):
    """ Function to extract data from a changeset file, accessed by ID
        input: the csid for a changeset
        output: a dictionary representation of the changeset
    """
    changeset = None
