...
from rules_based_class import RuleBased
...
#################################
#            single_label_experiment(nfolds, cs_train_path, resfile_name, outdir, vwargs, result_type,
#                                    ts_path=ts_test_path, print_misses=print_misses) # no train directory
def single_label_experiment(nfolds, cs_path, resfile_name, outdir, vwargs, result_type, ts_path=None, print_misses=False):
    # instantiate hybrid object
    suffix = 'single'
    clf = RuleBased(num_rules=6, filter_method='intersect')
    ...
    if(ts_path==None): # folds!
        ...
        for idx in range(len(folds)):
            ...
            labels, preds = get_scores(clf, X_train, y_train, X_test, y_test)
